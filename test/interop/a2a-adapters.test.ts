...
describe('A2A Adapters', () => {
  ...
  describe('createA2AProposal', () => {
    it('should create a proposal via negotiation client', () => {
      ...
      // Task descriptor is now a JSON string preserving skill + input for traceability
      const taskDescriptor = JSON.parse(proposal!.task);
      expect(taskDescriptor.protocol).toBe('a2a');
      expect(taskDescriptor.skillId).toBe('audit');
      expect(taskDescriptor.input).toBe('Audit this contract');
      expect(taskDescriptor.agentUrl).toBe('https://agent.example.com');
    ...
    it('should produce distinct proposals for same skill with different input', () => {
      const negotiation = new NegotiationClient();
      const id1 = createA2AProposal(negotiation, {
        card: SAMPLE_CARD,
        skillId: 'audit',
        input: 'Contract A',
        price: '100',
      ...
      const id2 = createA2AProposal(negotiation, {
        card: SAMPLE_CARD,
        skillId: 'audit',
        input: 'Contract B',
        price: '100',
      ...
      const p1 = JSON.parse(negotiation.get(id1)!.task);
      const p2 = JSON.parse(negotiation.get(id2)!.task);
      expect(p1.input).toBe('Contract A');
      expect(p2.input).toBe('Contract B');
      expect(p1.input).not.toBe(p2.input);
    });
  });
