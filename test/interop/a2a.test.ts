...
describe('A2AClient', () => {
  ...
  describe('_parseTask validation', () => {
    it('should reject task response with missing id', async () => {
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: async () => ({ status: 'working', messages: [], artifacts: [] }),
      });

      await expect(client.getTask('https://agent.example.com', 'x'))
        .rejects.toThrow('missing or empty "id"');
    });

    it('should reject task response with empty id', async () => {
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: async () => ({ id: '', status: 'working', messages: [], artifacts: [] }),
      });
    ...
    it('should reject task response with missing status', async () => {
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: async () => ({ id: 'task_1', messages: [], artifacts: [] }),
      });
    ...
    it('should reject task response with invalid status value', async () => {
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: async () => ({ id: 'task_1', status: 'banana', messages: [], artifacts: [] }),
      });
    ...
    it('should reject task response with non-array messages', async () => {
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: async () => ({ id: 'task_1', status: 'working', messages: 'not-an-array', artifacts: [] }),
      });
    ...
    it('should reject task response with non-array artifacts', async () => {
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: async () => ({ id: 'task_1', status: 'working', messages: [], artifacts: 'bad' }),
      });
    ...
    it('should accept valid task with all statuses', async () => {
      for (const status of ['submitted', 'working', 'input-required', 'completed', 'failed', 'canceled']) {
        mockFetch.mockResolvedValueOnce({
          ok: true,
          json: async () => ({ id: `task_${status}`, status, messages: [], artifacts: [] }),
        });
        const task = await client.getTask('https://agent.example.com', `task_${status}`);
        expect(task.id).toBe(`task_${status}`);
